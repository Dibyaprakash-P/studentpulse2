...
function gradeColor(g) { return (g && GRADE_COLORS[g.toUpperCase()]) || "var(--primary-yellow)"; }
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div className="grid grid-cols-1 md-grid-cols-2 lg-grid-cols-4" style={{ gap: 14, marginBottom: 24 }}>
        {[
          ...
          { label: "Subjects", value: subjects.length, icon: "🎓", color: "var(--primary-yellow)" },
          ...
          { label: "Files Uploaded", value: reports.reduce((a, r) => a + (r.files?.length || 0), 0), icon: "📎", color: "var(--primary-yellow)" },
        ].map((s, i) => (
      ...
      {subjects.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
          <button onClick={() => setFilter("All")} style={{
            ...
            border: `1px solid ${filter === "All" ? "var(--primary-yellow)" : "var(--border-subtle)"}`,
            background: filter === "All" ? "rgba(255,215,0,0.08)" : "transparent",
            color: filter === "All" ? "var(--primary-yellow)" : "var(--text-muted)",
            fontWeight: filter === "All" ? 600 : 400,
          }}>All</button>
          {subjects.map(s => (
            <button key={s} onClick={() => setFilter(s)} style={{
              padding: "7px 18px", borderRadius: 20, fontSize: "0.82rem", cursor: "pointer",
              border: `1px solid ${filter === s ? "var(--primary-yellow)" : "var(--border-subtle)"}`,
              background: filter === s ? "rgba(255,215,0,0.08)" : "transparent",
              color: filter === s ? "var(--primary-yellow)" : "var(--text-muted)",
                                ...
                                {r.files.map((f, idx) => (
                                  <div key={idx} style={{
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); downloadFile(f); }} style={{
                                      ...
                                      color: "var(--primary-yellow)", cursor: "pointer",
                                    }}>⬇ Download</button>
                                  </div>
                                ))}
                              </div>
