  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div className="grid grid-cols-1 md-grid-cols-3" style={{ gap: 14, marginBottom: 24 }}>
        ...
        <div className="glass-panel" style={{ padding: "16px 20px", borderLeft: "3px solid #FFD700" }}>
          <div style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: 4 }}>Completed</div>
          <div style={{ fontSize: "1.5rem", fontWeight: 800, color: "#FFD700" }}>{items.length - pending}</div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ overflow: "hidden" }}>
            <div className="glass-panel" style={{ marginBottom: 24, padding: "clamp(18px, 3vw, 26px)", borderLeft: `3px solid ${editingId ? "#FFD700" : "#3b82f6"}` }}>
              ...
              return (
                <motion.div key={h.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -80 }} layout
                  ...
                  <div className="glass-panel" style={{
                    padding: "16px 20px",
                    borderLeft: `3px solid ${h.completed ? "#FFD700" : dl.overdue ? "#ef4444" : priorityColor(h.priority)}`,
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                      ...
                      <button onClick={() => toggle(h.id)} style={{
                        ...
                        border: `2px solid ${h.completed ? "#FFD700" : "var(--border-light)"}`,
                        background: h.completed ? "#FFD700" : "transparent",
