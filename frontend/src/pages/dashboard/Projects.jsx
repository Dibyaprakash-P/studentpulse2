...
const STATUS_COLORS = { Planning: "var(--warning)", "In Progress": "var(--primary-yellow)", Completed: "var(--success)" };
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ overflow: "hidden" }}>
            <div className="glass-panel" style={{ marginBottom: 24, padding: "clamp(18px, 3vw, 26px)", borderLeft: `3px solid ${editingId ? "var(--primary-yellow)" : "var(--primary-yellow)"}` }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                      {form.tags.map(t => (
                        <span key={t} style={{
                          ...
                          background: "rgba(255,215,0,0.08)", color: "var(--primary-yellow)",
                          display: "flex", alignItems: "center", gap: 6,
                        }}>
                          {t}
                          <button onClick={() => removeTag(t)} style={{ background: "none", border: "none", color: "var(--primary-yellow)", cursor: "pointer", fontSize: "0.9rem" }}>×</button>
                        </span>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 12 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem", marginBottom: 6 }}>
                        ...
                        <span style={{ color: "var(--primary-yellow)", fontWeight: 700 }}>{p.progress}%</span>
                      </div>
                      <div style={{ height: 6, borderRadius: 6, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                        <motion.div initial={{ width: 0 }} animate={{ width: `${p.progress}%` }}
                          transition={{ duration: 0.8, ease: "easeOut" }}
                          style={{ height: "100%", borderRadius: 6, background: "linear-gradient(90deg, var(--primary-yellow), var(--primary-yellow))" }} />
                      </div>
                      <input type="range" min={0} max={100} step={5} value={p.progress}
                        onChange={e => setProgress(p.id, e.target.value)}
                        style={{
                          width: "100%", marginTop: 6,
                          background: `linear-gradient(to right, var(--primary-yellow) ${pct}%, var(--border-light) ${pct}%)`,
                        }} />
                    </div>
                    {/* Deadline badge */}
                    <div style={{
